...
def sample_motion_model_velocity_withT(particle,v,w,delta_t): # See page 124 in the book
    ...
    v_hat = v + randn(0,(0.2*(v**2))+(0.05*(w**2))) #Velocity with noise
   # print("v_hat:",v_hat)
    w_hat = w + randn(0,(0.2*(v**2))+(0.05*(w**2))) # angular velocity with noise
   #print("w_hat:",w_hat)
    epsilon = randn(0,0.2*(v**2)+0.05*(w**2)) # Random term
